...
Original file is located at
    https://colab.research.google.com/drive/1T-qfTXsuwj8CPvso-R6dDXRcfO5NPBPw
"""

# Install all the required Packages

# !pip install pyotp --quiet
# !pip install logzero --quiet
...
# Fetch the entire instructions tradable in the day
import pandas as pd
import requests
...
#       "expirydate":"24APR2025"
#   }
#   df = df + (smartApi.optionGreek(params)['data'])
#   time.sleep(0.3)

# pd.DataFrame(df)

print(stock_symbols.index('ABB'))

# token_df.to_excel('output.xlsx', index=False)
...
#     "BANKINDIA", "BERGEPAINT", "BEL", "BHARATFORG", "BHEL", "BPCL", "BHARTIARTL", "BIOCON",
#     "BOSCHLTD", "BRITANNIA", "CESC", "CGPOWER", "CANBK", "CDSL", "CHAMBLFERT", "CHOLAFIN",
#     "CIPLA", "COALINDIA", "COFORGE", "COLPAL", "CAMS", "CONCOR", "CROMPTON", "CUMMINSIND",
#     "CYIENT", "DLF", "DABUR", "DALBHARAT", "DEEPAKNTR", "DELHIVERY", "DIVISLAB", "DIXON",
#     "DRREDDY", "EICHERMOT", "ESCORTS", "EXIDEIND", "NYKAA", "GAIL", "GMRAIRPORT", "GLENMARK",
#     "GODREJCP", "GODREJPROP", "GRANULES", "GRASIM", "HCLTECH", "HDFCAMC", "HDFCBANK",
#     "HDFCLIFE", "HFCL", "HAVELLS", "HEROMOTOCO", "HINDALCO", "HAL", "HINDCOPPER",
#     "HINDPETRO", "HINDUNILVR", "HUDCO", "ICICIBANK", "ICICIGI", "ICICIPRULI", "IDFCFIRSTB",
#     "IIFL", "IRB", "ITC", "INDIANB", "IEX", "IOC", "IRCTC", "IRFC", "IREDA", "IGL",
#     "INDUSTOWER", "INDUSINDBK", "NAUKRI", "INFY", "INDIGO", "JKCEMENT", "JSWENERGY",
#     "JSWSTEEL", "JSL", "JINDALSTEL", "JIOFIN", "JUBLFOOD", "KEI", "KPITTECH", "KALYANKJIL",
#     "KOTAKBANK", "LTF", "LTTS", "LICHSGFIN", "LTIM", "LT", "LAURUSLABS", "LICI", "LUPIN",
#     "MRF", "LODHA", "MGL", "M&MFIN", "M&M", "MANAPPURAM", "MARICO", "MARUTI", "MFSL",
#     "MAXHEALTH", "MPHASIS", "MCX", "MUTHOOTFIN", "NBCC", "NCC", "NHPC", "NMDC", "NTPC",
#     "NATIONALUM", "NESTLEIND", "OBEROIRLTY", "ONGC", "OIL", "PAYTM", "OFSS", "POLICYBZR",
#     "PIIND", "PAGEIND", "PATANJALI", "PERSISTENT", "PETRONET", "PIDILITIND", "PEL",
#     "POLYCAB", "POONAWALLA", "PFC", "POWERGRID", "PRESTIGE", "PNB", "RBLBANK", "RECLTD",
#     "RELIANCE", "SBICARD", "SBILIFE", "SHREECEM", "SJVN", "SRF", "MOTHERSON", "SHRIRAMFIN",
#     "SIEMENS", "SOLARINDS", "SONACOMS", "SBIN", "SAIL", "SUNPHARMA", "SUPREMEIND", "SYNGENE",
#     "TATACONSUM", "TITAGARH", "TVSMOTOR", "TATACHEM", "TATACOMM", "TCS", "TATAELXSI",
#     "TATAMOTORS", "TATAPOWER", "TATASTEEL", "TATATECH", "TECHM", "FEDERALBNK", "INDHOTEL",
#     "PHOENIXLTD", "RAMCOCEM", "TITAN", "TORNTPHARM", "TORNTPOWER", "TRENT", "TIINDIA",
...
def get_monthly_expiry(df, equity_data, names):
    ...
    print(f"im in heres the length of ce is : {len(ce_options)}")
    pe_options = df_filtered[df_filtered['symbol'].str.endswith('PE')]
    print(f"im in here the length of pe is : {len(pe_options)}")
...
# def get_top_option_contracts(condition_type, df):
#     worksheet_name = condition_type
#     try:
#         worksheet = sheet.worksheet(worksheet_name)  # Try to get the worksheet
#     except WorksheetNotFound:
#         worksheet = sheet.add_worksheet(title=worksheet_name, rows="1000", cols="32")  # Create if not found

#     HEADERS_2 = ["Name", "CMP", "Open_price", "percent_change"]

#     # Define filters based on condition_type
#     match condition_type:
#         case "premium_gt_1":
#             df = df[df['Premium_Rate'] > 1].sort_values(by=["Premium_Rate"],ascending=False).head(20)
#             # ✅ **Get the correct range for batch update**
#             header_range = f"A1:{rowcol_to_a1(1, len(HEADERS))}"
#             data_range = f"A2:{rowcol_to_a1(len(df) + 1, len(HEADERS))}"

#             worksheet.batch_update([
#                 {"range": header_range, "values": [HEADERS]},
#                 {"range": data_range, "values": df.values.tolist()},
#             ])

#         case "premium_2_to_5":
#             df = df[(df['Premium_Rate'] > 2) & (df['Premium_Rate'] < 5)].sort_values(by=["Premium_Rate"],ascending=False).head(20)
#             # ✅ **Get the correct range for batch update**
#             header_range = f"A1:{rowcol_to_a1(1, len(HEADERS))}"
#             data_range = f"A2:{rowcol_to_a1(len(df) + 1, len(HEADERS))}"
...
#         case "cover_upto_10":
#             df = df[(df['Cover'] < 10) ].sort_values(by=["Cover"],ascending=False).head(20)
#             # ✅ **Get the correct range for batch update**
#             header_range = f"A1:{rowcol_to_a1(1, len(HEADERS))}"
#             data_range = f"A2:{rowcol_to_a1(len(df) + 1, len(HEADERS))}"
...
#             ])
#         case "cover_10_to_20":
#             df = df[(df['Cover'] > 10) &  (df['Cover'] < 20)].sort_values(by=["Cover"],ascending=False).head(20)
#             # ✅ **Get the correct range for batch update**
#             header_range = f"A1:{rowcol_to_a1(1, len(HEADERS))}"
#             data_range = f"A2:{rowcol_to_a1(len(df) + 1, len(HEADERS))}"
...
#         case "cover_above_20":
#             df = df[ (df['Cover'] > 20)].sort_values(by=["Cover"],ascending=False).head(20)
#             # ✅ **Get the correct range for batch update**
#             header_range = f"A1:{rowcol_to_a1(1, len(HEADERS))}"
#             data_range = f"A2:{rowcol_to_a1(len(df) + 1, len(HEADERS))}"
...
#             df['Open_price'] != 0,
#             ((df['CMP'] - df['Open_price']) / df['Open_price']) * 100,
#             0
#         )
#             top_20_unique = (
#                 df
#                 .drop_duplicates(subset='percent_change')
#                 .sort_values(by='percent_change', ascending=False)
#                 .head(20)  # Take top 20 unique values
#             )
#             top_20_unique = top_20_unique[['Name','CMP','Open_price','percent_change']]
#             # ✅ **Get the correct range for batch update**
#             header_range = f"A1:{rowcol_to_a1(1, len(HEADERS_2))}"
#             data_range = f"A2:{rowcol_to_a1(len(top_20_unique) + 1, len(HEADERS_2))}"

#             worksheet.batch_update([
#                 {"range": header_range, "values": [HEADERS_2]},
#                 {"range": data_range, "values": top_20_unique.values.tolist()},
#             ])
...
#                 .head(20)  # Take top 20 unique values
#             )
#             bottom_20_unique = bottom_20_unique[['Name','CMP','Open_price','percent_change']]
#             # ✅ **Get the correct range for batch update**
#             header_range = f"A1:{rowcol_to_a1(1, len(HEADERS_2))}"
#             data_range = f"A2:{rowcol_to_a1(len(bottom_20_unique) + 1, len(HEADERS_2))}"

#             worksheet.batch_update([
#                 {"range": header_range, "values": [HEADERS_2]},
#                 {"range": data_range, "values": bottom_20_unique.values.tolist()},
#             ])

#         case _:
#             raise ValueError("Invalid condition type provided.")
...
# test_data['percent_change'] = np.where(
#     test_data['Open_price'] != 0,
#     ((test_data['CMP'] - test_data['Open_price']) / test_data['Open_price']) * 100,
#     0
# )
# top_20_unique = (
#     test_data
#     .drop_duplicates(subset='percent_change')        # Remove duplicates
#     .sort_values(by='percent_change', ascending=False)  # Sort descending
#     .head(20)  # Take top 20 unique values
# )
# top_20_unique[['Name','CMP','Open_price','percent_change']]





# test_data.sort_values(by=["percent_change"],ascending=False).head(20)
...
# def write_to_google_sheet(worksheet_name):
#     global test_data
#     try:
#         worksheet = sheet.worksheet(worksheet_name)  # Try to get the worksheet
#     except WorksheetNotFound:
#         worksheet = sheet.add_worksheet(title=worksheet_name, rows="10000", cols="32")  # Create if not found

#     if worksheet is None:
#         print("⚠️ Worksheet not initialized!")
#         return
...
#     test_data = df
#     get_top_option_contracts('premium_gt_1',df)
#     get_top_option_contracts('premium_2_to_5',df)
#     get_top_option_contracts('cover_upto_10',df)
#     get_top_option_contracts('cover_10_to_20',df)
#     get_top_option_contracts('cover_above_20',df)
#     get_top_option_contracts('cover_atmost_-10',df)
#     get_top_option_contracts('cover_-10_to_-20',df)
#     get_top_option_contracts('cover_below_-20',df)
#     get_top_option_contracts('cmp_gainers',df)
#     get_top_option_contracts('cmp_losers',df)

#     # ✅ **Get the correct range for batch update**
#     header_range = f"A1:{rowcol_to_a1(1, len(HEADERS))}"
#     data_range = f"A2:{rowcol_to_a1(len(rows) + 1, len(HEADERS))}"
...
# option_batches = [option_tokens[i:i + BATCH_SIZE] for i in range(0, len(option_tokens), BATCH_SIZE)]

# # Split option batches between ws2 and ws3
# half_batches = math.ceil(len(option_batches) / 2)
# ws2_batches = option_batches[:half_batches]  # First half for ws2
# ws3_batches = option_batches[half_batches:]  # Second half for ws3

# def subscribe_batches(ws, ws_id, batch_list):
#     """Helper function to subscribe batches for a given WebSocket."""
#     for i, batch in enumerate(batch_list):
#         logger.info(f"{ws_id}: Subscribing batch {i+1}/{len(batch_list)} with {len(batch)} tokens")
#         batch_subscription = [{"exchangeType": 2, "tokens": batch}]
#         ws.subscribe(f"{ws_id}_batch_{i+1}", mode_2, batch_subscription)
...
#     with option_data_lock:
#         option_data[token]["option"] = update
...
#     while True:
#         try:
#             token, update = update_queue.get(timeout=1)
#             # logger.debug(f"Processing update for token {token}")
#             process_update(token, update)
#             update_queue.task_done()
#             # logger.debug(f"Finished processing token {token}")
#         except queue.Empty:
#             logger.info("Worker thread exiting (queue empty)")
#             break
#         except Exception as e:
#             logger.error(f"Worker error processing token: {e}")
...
def daily_sheet_writer():
    ...
    while True:
        ...
        if now.hour == 15 and now.minute == 45:  # 3:45 PM IST
            ...
            write_to_google_sheet(sheet_name)  # Pass sheet name to function
...
# First WebSocket tokens (Tick Data)
option_list = [
    {
        "exchangeType": 2,
...
def on_open_3(wsapp):
    """WebSocket 3 - Process second half of option tokens in order."""
    logger.info("Options WebSocket 3 Opened")
    threading.Thread(target=cycle_subscriptions, args=(sws3, correlation_id_3, copy.deepcopy(ws3_batches)), daemon=True).start()
...
sws3 = SmartWebSocketV2(AUTH_TOKEN, API_KEY, CLIENT_CODE, FEED_TOKEN)
sws3.on_open = on_open_3
sws3.on_error = on_error
sws3.on_close = on_close
